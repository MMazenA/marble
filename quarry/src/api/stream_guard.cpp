...
StreamGuard::StreamGuard(net::io_context &ioc)
    : m_ioc(ioc), m_tls_ios(nullptr),
      m_stream(std::in_place_type<tcp_stream>, ioc) {};

StreamGuard::StreamGuard(std::string host, net::io_context &ioc,
                         ssl::context &tls_ioc)
    : m_host(std::move(host)), m_ioc(ioc), m_tls_ios(&tls_ioc),
...
void StreamGuard::set_sni_hostname(std::string &host_str) {
  auto tcp_handler = [&](tcp_stream &stream) { return; };
