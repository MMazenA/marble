...
// Don't create naming conflicts with recent libc++ which defines
// std::iter_reference_t in <iterator> but doesn't yet provide a <ranges>
// header.
...
struct _fn {
  ...
  auto operator()(_Range &&__r) const noexcept(noexcept(__r.begin()))
      -> decltype(__r.begin()) {
    return __r.begin();
  }
  ...
  auto operator()(_Range &&__r) const noexcept(noexcept(begin(__r)))
      -> decltype(begin(__r)) {
    return begin(__r);
  }
};
...
struct _fn {
  ...
  auto operator()(_Range &&__r) const noexcept(noexcept(__r.end()))
      -> decltype(__r.end()) {
    return __r.end();
  }
  ...
  auto operator()(_Range &&__r) const noexcept(noexcept(end(__r)))
      -> decltype(end(__r)) {
    return end(__r);
  }
};
...
public:
  ...
  _T &construct(_Args &&...__args) noexcept(
      std::is_nothrow_constructible_v<_T, _Args...>) {
...
template <typename _Ref, typename _Value = std::remove_cvref_t<_Ref>,
          typename _Allocator = use_allocator_arg>
class generator;

template <typename _Alloc> class __promise_base_alloc {
  static constexpr std::size_t
  __offset_of_allocator(std::size_t __frameSize) noexcept {
    return __aligned_allocation_size(__frameSize, alignof(_Alloc));
  }

  static constexpr std::size_t
  __padded_frame_size(std::size_t __frameSize) noexcept {
    return __offset_of_allocator(__frameSize) + sizeof(_Alloc);
  }

  static _Alloc &__get_allocator(void *__frame,
                                 std::size_t __frameSize) noexcept {
    return *reinterpret_cast<_Alloc *>(static_cast<char *>(__frame) +
                                       __offset_of_allocator(__frameSize));
  }

public:
  template <typename... _Args>
  static void *operator new(std::size_t __frameSize, std::allocator_arg_t,
                            _Alloc __alloc, _Args &...) {
    void *__frame = __alloc.allocate(__padded_frame_size(__frameSize));

    // Store allocator at end of the coroutine frame.
    // Assuming the allocator's move constructor is non-throwing (a requirement
    // for allocators)
    ::new (static_cast<void *>(std::addressof(
        __get_allocator(__frame, __frameSize)))) _Alloc(std::move(__alloc));

    return __frame;
  }

  template <typename _This, typename... _Args>
  static void *operator new(std::size_t __frameSize, _This &,
                            std::allocator_arg_t, _Alloc __alloc, _Args &...) {
    return __promise_base_alloc::operator new(__frameSize, std::allocator_arg,
                                              std::move(__alloc));
  }
  ...
  static void operator delete(void *__ptr, std::size_t __frameSize) noexcept {
    ...
    __alloc.~Alloc();
    __localAlloc.deallocate(static_cast<std::byte *>(__ptr),
                            __padded_frame_size(__frameSize));
  }
};
...
template <typename _Ref> struct __generator_promise_base {
  ...
  // This member is lazily constructed by the
  // __yield_sequence_awaiter::await_suspend() method if this generator is used
  // as a nested generator.
  ...
  std::suspend_always
  yield_value(_Ref &&__x) noexcept(std::is_nothrow_move_constructible_v<_Ref>) {
  ...
  std::suspend_always
  yield_value(_T &&__x) noexcept(std::is_nothrow_constructible_v<_Ref, _T>) {
  ...
  yield_value(std::ranges::elements_of<generator<_Ref, _OValue, _OAlloc>>
                  __g) noexcept {
...
template <typename _Generator, typename _ByteAllocator,
          bool _ExplicitAllocator = false>
struct __generator_promise;

template <typename _Ref, typename _Value, typename _Alloc,
          typename _ByteAllocator, bool _ExplicitAllocator>
struct __generator_promise<generator<_Ref, _Value, _Alloc>, _ByteAllocator,
                           _ExplicitAllocator>
    final : public __generator_promise_base<_Ref>,
            public __promise_base_alloc<_ByteAllocator> {
  __generator_promise() noexcept
      : __generator_promise_base<_Ref>(
...
template <typename _Alloc>
using __byte_allocator_t = typename std::allocator_traits<
    std::remove_cvref_t<_Alloc>>::template rebind_alloc<std::byte>;
...
struct coroutine_traits<generator<_Ref, _Value>, _Args...> {
  using promise_type =
      __generator_promise<generator<_Ref, _Value>, std::allocator<std::byte>>;
};
...
struct coroutine_traits<generator<_Ref, _Value>, allocator_arg_t, _Alloc,
                        _Args...> {
private:
...
public:
  using promise_type =
      __generator_promise<generator<_Ref, _Value>, __byte_allocator,
                          true /*explicit Allocator*/>;
};

// Type-erased allocator with std::allocator_arg parameter (non-static member
// functions)
template <typename _Ref, typename _Value, typename _This, typename _Alloc,
          typename... _Args>
struct coroutine_traits<generator<_Ref, _Value>, _This, allocator_arg_t, _Alloc,
                        _Args...> {
private:
  using __byte_allocator = __byte_allocator_t<_Alloc>;
...
public:
  using promise_type =
      __generator_promise<generator<_Ref, _Value, _Alloc>, __byte_allocator>;
};
...
public:
  ...
  private:
    ...
    explicit iterator(__promise_base *__promise,
                      std::coroutine_handle<> __coro) noexcept
